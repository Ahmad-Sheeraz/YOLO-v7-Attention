...
###### MobileSR Transformer ######

class SelfAttn(nn.Module):
    def __init__(self, c2, num_heads=8, bias=False):
        super(SelfAttn, self).__init__()
        self.num_heads = num_heads
        head_dim = c2 // num_heads
        self.scale = head_dim ** -0.5

        self.qkv = nn.Linear(c2, c2*3, bias=bias)
        self.proj_out = nn.Linear(c2, c2)

    def forward(self, x):
        b, N, c = x.shape
        
        qkv = self.qkv(x).chunk(3, c2=-1)
...
class Mlp(nn.Module):
    def __init__(self, c2, mlp_ratio=4):
        super(Mlp, self).__init__()
        hidden_features = c2 * mlp_ratio
        
        self.fc = nn.Sequential(
            nn.Linear(c2, hidden_features),
            nn.GELU(),
            nn.Linear(hidden_features, c2)
        )
...
class MobileTransformer(nn.Module):
    def __init__(self, c2, num_heads=4, window_size=8, mlp_ratio=4, qkv_bias=False):
        super(MobileTransformer, self).__init__()
        self.window_size=window_size
        self.pos_embed = nn.Conv2d(c2, c2, 3, padding=1, groups=c2)
        
        self.norm1 = nn.LayerNorm(c2)
        self.attn = SelfAttn(c2, num_heads, qkv_bias)
        self.norm2 = nn.LayerNorm(c2)
      
        self.mlp = Mlp(c2, mlp_ratio)
...
class ResBlock(nn.Module):
    def __init__(self, c2, ratio=4):
        super(ResBlock, self).__init__()
    
        self.net = nn.Sequential(
            nn.Conv2d(c2, c2*ratio, 1, 1, 0),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(c2*ratio, c2*ratio, 3, 1, 1, groups=c2*ratio),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(c2*ratio, c2, 1, 1, 0),
        )
...
class BaseBlock(nn.Module):
    def __init__(self, c1, c2, num_heads=8, window_size=8, ratios=[1, 2, 2, 4, 4], qkv_bias=False):
        ...
        for ratio in ratios:
            self.layers.append(nn.ModuleList([ 
                MobileTransformer(c2, num_heads, window_size, ratio, qkv_bias),
                ResBlock(c2, ratio)
            ]))
...
class DAB(nn.Module):
    def __init__(
        ...
        bias=True, bn=False, act=nn.ReLU(True), conv=conv):

        super(DAB, self).__init__()
        modules_body = []
        for i in range(2):
            modules_body.append(conv(n_feat, n_feat, kernel_size=3, bias=bias))
